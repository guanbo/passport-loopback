...
function Strategy(options, verify) {
  ...
  if (!options.authorizationURL) { throw new TypeError('LoopbackStrategy requires a authorizationURL option'); }
	...
	if (typeof this._headerFields === 'string') {
		this._headerFields = [this._headerFields];
		console.log(this._headerFields);
	}
	if (typeof this._tokenFields === 'string') {
		this._tokenFields = [this._tokenFields];
	}
}
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  this._headerFields.some(field=>{
    token = req.headers[field] || req.headers[field.toLowerCase()];
    return !!token;
  });
